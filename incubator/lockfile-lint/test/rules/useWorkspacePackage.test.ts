...
import { useWorkspacePackage } from "../../src/rules/useWorkspacePackage.ts";
...
describe("rules:useWorkspacePackage()", () => {
  const lockfile = {
    ...
    "@rnx-kit/lockfile-lint@workspace:*": {
      package: "@rnx-kit/lockfile-lint",
      specifiers: ["workspace:*"],
      resolution: "0.0.0-use.local",
    },
    "@rnx-kit/lockfile-lint@npm:1.0.0": {
      package: "@rnx-kit/lockfile-lint",
  ...
  it("flags packages that should have resolved locally", () => {
    const packages = ["@rnx-kit/lockfile-lint"];
    ...
    const cases: [keyof typeof lockfile, number][] = [
      ...
      ["@rnx-kit/lockfile-lint@npm:1.0.0", 1],
      ["@rnx-kit/lockfile-lint@workspace:*", 0],
    ];
