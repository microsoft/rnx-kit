...
import { getPluginKey, pluginTraceFactory } from "./plugins.ts";
...
export function getBabelConfig(
  ...
  const { traceBabelPlugins, tracePerformance, configCallerMixins } = settings;
  ...
  const wrapPluginVisitorMethod =
    traceBabelPlugins && tracePerformance
      ? pluginTraceFactory(trace)
      : undefined;
  ...
  // return the resolved config, filtering the plugins if necessary
  return filterConfigPlugins(babelRc, settings.configDisabledPlugins);
}
