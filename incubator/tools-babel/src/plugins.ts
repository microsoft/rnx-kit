...
import { getTrace, isTrackingEnabled } from "@rnx-kit/tools-performance";

export const PLUGIN_DOMAIN = "babel-plugin";

export function shouldInstrumentPlugins() {
  return isTrackingEnabled(PLUGIN_DOMAIN, "high");
}
...
export const pluginTraceFactory = (() => {
  ...
  return () => {
    ...
    // this will only be enabled if we are requesting high frequency tracing
    const trace = getTrace(PLUGIN_DOMAIN, "high");
    return (factory = <T extends AnyFuncType>(
      ...
      function thisWrapper(this: ThisParameterType<T>, ...args: Parameters<T>) {
        return trace(key, () => fn.apply(this, args));
      }
      return thisWrapper;
    });
  };
