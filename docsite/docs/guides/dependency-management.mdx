...
The first order of business is on-boarding your React Native packages to the
...
Once on-boarding is done, you're ready to set up automation.
...
[Gulp](https://gulpjs.com), you can go for a more sophistocated integration. The
...
on-boarded yet. For those, `--vigilant [version]` controls the target React
...
an old release), you can use the dependency manager to do the heavy lifting.
...
Next, configure each of your on-boarded React Native packages to use the list.
...
Was this guide helpful? Spread the word on [Twitter](https://twitter.com):
