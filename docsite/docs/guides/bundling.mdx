...
compensate for Metro's lack of symlink support. The mappings look something like
this:
...
Its time to put together an `rnx-bundle` command for your app. Start with
...
If your curious about the rest of the command-line parameters, you can find the
...
Next, put together an `rnx-start` command for your app. Parameters aren't
typically needed for this command. However, you should take a look at the
[CLI server documentation](/docs/tools/cli#command-line-overrides-1) and decide
for yourself.
...
practice is to have one bundle script per platform and an "umbrella" bundle
script which runs them all.
...
knows what to name the bundle file, and where to store assets, so the native
host app can find them at runtime. And it knows how these things vary across
each platform.

All of this knowledge is encoded in the command-lines you've written for your
app.

[App configuration](/docs/tools/config) is a declarative way to express these
decisions as _defaults_ for how your app is bundled. All the knowledge is
captured in one easy-to-read format, without duplication.
...
```sh
react-native rnx-bundle --entryFile test.js
```

Now its time to convert your `rnx-bundle` commands into app defaults. Use the
...
Do the same thing for your `rnx-start` commands, but only if you added
command-line parameters. Use the
[ServerConfig schema](/docs/tools/config#serverconfig-inherits-bundlerplugins)
as a reference. If you don't specify any `ServerConfig`, the server will use
...
TypeScript validation is **on** by default. You can control it with
[typescriptValidation](/docs/tools/config#bundlerplugins).
...
**Bonus:** When tree shaking is enabled, your app will bundle a lot faster! Tree
shaking runs on `esbuild`, a native-code tool that is optimized for speed.

Tree shaking is _off_ by default. You can control it with
[treeShake](/docs/tools/config#bundlerplugins).
...
in it? Yes, this is a thing that happens. Sometimes, the duplicates are same
version of the package! :package: :package: :package:
...
Duplicate detection is **on** by default. You can control it with
[detectDuplicateDependencies](/docs/tools/config#bundlerplugins).
...
with [detectCyclicDependencies](/docs/tools/config#bundlerplugins).
...
`throwOnError` to `false` in the `typescriptValidation` configuration object.
It's also a good idea to open a new issue for tracking the type errors, and as a
reminder to re-enable `throwOnError`. In the mean time, keeping the errors
...
satisfy all constraints. That will lead to only one copy being downloaded and
installed in your repo.

Below are some helpful tricks you can try:

- (Easy) Make sure your monorepo packages all depend on the same version of the
  duplicate package.
- (Easy) If you are using yarn, run the `yarn-deduplicate` package to collapse
  away duplicate entries in your yarn.lock file. Surprisingly, these aren't
...
- (Hard) If you are using pnpm, trace the duplicates through the maze of parent
  ...
  this difference.
- (Dangerous, Easy). Force yarn/pnpm to use the same version of the duplicate
  package. yarn has a `resolutions` property which does this. pnpm has a
  `readPackage` hook which you can use to do this. Both are dangerous because
  **you can break the packages that depend on the package version you are
  changing**.

If you can't get rid of the duplicate package, and your app seems to be working,
you can choose to live with it. Add it to the `ignoredModules` array in the
`detectDuplicateDependencies` configuration object.
...
- If any of the packages live in your repo, you refactor the code to break the
  cycle.
- Even if they're all external packages, you can submit a PR, though it may take
  time to be accepted and published. If you're impatient, you can use
  `patch-package` while you wait for the PR to land.
- Try looking for a different package that does roughly what you want.
- Duplicate some of the code in your own repo, enough for you to be able to
  break the cycle.

If none of this is appealing, you can take a closer look at the code involved
and understand if and how an infinite loop might occur. If you can avoid
triggering it, then document that in your code, and live with the cycle. Set
`throwOnError` to `false` in the `detectCyclicDependencies` configuration
object.

## Integrating with PRs and Builds

You're in the home stretch. You've done the hard part. Now you'll add some
automation to guard your app during future development. :shield:
...
If you're using a task runner like Nx or Lage or Gulp, you can go for a more
...
[a discussion topic](https://github.com/microsoft/rnx-kit/discussions), or
