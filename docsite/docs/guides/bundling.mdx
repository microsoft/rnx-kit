...
This page needs to be tested in a simple repo like rnts-example.
...
This turns on bundling with a
[default configuration](../packages/config#default-configuration).
...
[config schema](../packages/config#schema) to see what's available.
...
[config schema](../packages/config#schema), mapping what fits.
...
If you use [Lerna](https://lerna.js.org), you can run one command from the root
of your repo to bundle all packages:
...
advantage being that `bundle` runs as its own task with isolated logging and
error handling. And, you can run it in parallel with other tasks.
...
Turn on the
[duplicate dependency checker](../packages/metro-plugin-duplicates-checker):
...
Find them using the
[cyclic dependency detector](../packages/metro-plugin-cyclic-dependencies-detector):
...
This [CLI](../packages/cli) command is very similar to `react-native start`.
...
Take a look at the
[config schema](http://localhost:3000/rnx-kit/docs/packages/config#schema) to
