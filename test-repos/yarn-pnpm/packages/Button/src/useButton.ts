...
import type { ButtonInfo, ButtonProps } from "./Button.types";
...
export const useButton = (props: ButtonProps): ButtonInfo => {
  ...
  const pressable = usePressableState({
    ...rest,
    onPress: onClickWithFocus,
    onBlur: shouldOnlyFireIfPressed ? onBlurInner : onBlur,
  });
  ...
  const [baseWidth, setBaseWidth] = React.useState<number | undefined>(
    undefined
  );
  ...
  return {
    props: {
      ...
      onAccessibilityTap:
        ...
        (!hasTogglePattern ? props.onClick : undefined),
