...
> **Note**
>
> This document conveys what we want to achieve with rnx-kit. Whether we have
> resources to fund all the efforts remains to be seen. As such, we make no
> guarantees on timelines or otherwise. It represents solely our stance on React
> Native and is not representative of the community as a whole.

## Motivation

The big draw of React Native is the promise of increased productivity with a
web-like development experience for native while maintaining a seamless
integration with the target platform, all for a little overhead over a pure
native solution. While it's true that it has brought things like the ability to
see your changes as you write the code and being able to share business logic
with web, the tools we use have not become any easier to work with. In fact, it
has only become more complex as we need to work with platform specific tools in
addition to JS ones. This complexity is what drives stakeholders towards
alternatives despite what performance numbers indicate.

## Goal: Bring the web's best developer experience to React Native

If we look at how to get started with web development, we can reduce them down
...
That's only three steps get started! It can be further reduced if we consider
the use of virtual environments like [GitHub Codespaces][] which manage setting
up the tooling mostly automagically.

Web browsers include almost everything you might need. The standard APIs that
can be used to access audio, graphics, the file system, and many others.
Development tools are embedded directly right next to your app. Web browsers
basically have all the batteries included. Compare that to how one would get
...
experience to end users. This makes React Native, with all its complexities, a
hard choice to make. It's not enough to talk about performance numbers and great
user experiences: we also need to court developers with a great developer
experience. In the next sections, we will outline what we are currently doing
today and what we need to invest in to achieve this goal.

## React Native's "web browser"

One of the reasons the web is easy to get started with is because you only need
a web browser. Practically all computers have one these days. Upgrading them is
...
that point in time. Every time a new versions of React Native is released, they
...
This effort predates rnx-kit, and we've achieved many of the things we've aimed
for with [`react-native-test-app`][], but still need the last push over the
finishing line.
...
>   - It is maintained by the very same folks maintaining rnx-kit
> - `react-native-test-app` currently does not support/target web.
> - We want `react-native-test-app` to be the best choice for developers,
>   whether they are working on a library or a standalone app.
...
> - This section only relates to the Web APIs. For the DOM counterpart, please
>   refer to the [React DOM For Native RFC][].

The final piece of the browser puzzle is a standard API. Today, web developers
can reach out to Web APIs and assume that their app will function across many
platforms. That is not the case for React Native developers. If the API you're
looking for is not immediately available in `react-native`, you will have to
look for modules written by the community. Maybe you'll find one in [React
Native Directory][], or maybe you will have to write one yourself. We want to
shift the mindset from "what packages to use" to "what APIs are available".

Standard APIs is what will make the "web browser" effort truly ubiquitous. We
cannot provide a downloadable app for development without this.
...
centralized design allows it to align dependencies across many packages both
within a repository and across repositories.

### JS bundling

Here we need to align with Meta and use [Metro][]. It is what Meta uses
...
Metro and [`@react-native-community/cli`][].
...
would require a re-architecture. The necessary changes is too invasive. Instead,
we chose to write a plugin and a few minor changes to Metro to enable it. This
plugin currently requires a recent version of Metro (one that ships with our
patches), configuration of two different tools (Babel and Metro), and logic to
enable/disable some of the configuration based on the current mode. This is all
too much for users that just want to get started on their actual work. With a
CLI, we can abstract away complexities like this behind a single flag. This is
why having a CLI to tie all these things together is important. If you look at
the current [list of tools][], we're building up quite a repertoire.
...
any other tools than Node. This is essential to lower the barriers to entry,
...
have high hopes for great things coming out of the recently spun up [Developer
Experience Working Group][].

## Community

Last but not least, it is hard to understate the importance of the React Native
community. As stewards of rnx-kit, we want to be active contributors in this
community — not break or split it.
