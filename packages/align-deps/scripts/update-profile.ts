...
async function main({
  ...
  const allVersions: string[] = Object.keys(preset)
    .sort((lhs, rhs) =>
      semverCompare(
        ensureNonNull(semverCoerce(lhs)),
        ensureNonNull(semverCoerce(rhs))
      )
    )
    .reverse();
...
async function parseArgs(): Promise<Options> {
  ...
  if (options.releaseCandidate) {
    ...
    if (!isFulfilled(latest, "latest") || !isFulfilled(next, "next")) {
      throw new Error(
        "Failed to fetch latest and next version of 'react-native'"
      );
    }
