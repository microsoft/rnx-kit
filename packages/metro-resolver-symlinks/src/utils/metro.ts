...
import type { ModuleHash, Options, ResolutionContextCompat } from "../types.ts";
...
function computeSha1(filePath: string): ModuleHash {
  // The hash needs to match what Metro uses to store transformed modules
  // otherwise we'll get cache misses:
  // https://github.com/react/metro/blob/v0.87.0/packages/metro/src/DeltaBundler/Worker.flow.js#L118
    ...
    DependencyGraph.prototype.getSha1 = function (filePath: string): string {
      try {
        ...
        if (e instanceof ReferenceError || isSha1Error(e)) {
          return computeSha1(filePath).sha1;
        }
