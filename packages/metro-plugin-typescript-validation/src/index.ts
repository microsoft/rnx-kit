...
function getModuleRoot(module: string): string {
  ...
  return p.slice(index + module.length);
}

function readTsConfig(projectRoot: string) {
  const p = path.join(projectRoot, "tsconfig.json");
  return JSON.parse(fs.readFileSync(p, { encoding: "utf8" }));
}

function writeMetroTsConfig(projectRoot: string, tsconfig: object): string {
...
function runTypeScriptCompiler(projectPath: string) {
