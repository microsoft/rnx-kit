...
export function traverseDirectory(
  ...
  exclusionListDirs: string[] = [],
  recursive = true
) {
  log.verbose(
    "traverseDirectory",
    `Entering ${rootAbsPath}${fs_path.sep}${relPath}`
  );
  ...
  if (isRegularFile(path)) {
    ...
    children.forEach((childpath: string) => {
      ...
      if (
        ...
        log.verbose(
          "traverseDirectory",
          `Ignoring ${rootAbsPath}${fs_path.sep}${relChildPath} as it's exclusionListed.`
        );
        return;
      }
...
export function copyFile(absSourcePath: string, absDestinationPath: string) {
  ensureDirOfPathExists(absDestinationPath);
  fse.copyFileSync(absSourcePath, absDestinationPath);
}
